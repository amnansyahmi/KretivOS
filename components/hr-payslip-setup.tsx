  ...
  return <div className="space-y-5">
    <Card className="overflow-hidden border-amber-200 bg-amber-50/80">
      <CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-start">
        ...
        <div className="flex items-center gap-2 rounded-full bg-card px-3 py-1.5 text-[10px] font-semibold text-amber-800"><ShieldCheck className="h-3.5 w-3.5" />Protected release</div>
      </CardContent>
    </Card>

    <div className="grid gap-5 xl:grid-cols-[360px_1fr]">
      <Card className="border-black/8 bg-white/90"><CardContent className="p-5">
        <div className="flex items-center gap-2"><WalletCards className="h-4 w-4 text-accent" /><h2 className="font-semibold">Payslip setup</h2></div>
        ...
        <label className="mt-5 block text-xs font-medium text-foreground-soft">Employee<select value={employeeId} onChange={(event) => setEmployeeId(event.target.value)} className="mt-2 h-11 w-full rounded-xl border bg-card px-3 text-sm outline-none">{active.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
        <label className="mt-4 block text-xs font-medium text-foreground-soft">Pay period<input value={period} onChange={(event) => setPeriod(event.target.value)} className="mt-2 h-11 w-full rounded-xl border bg-card px-3 text-sm outline-none" /></label>

        <div className="mt-5 space-y-2 text-xs">
          {["Basic salary", "Allowances", "Overtime", "Bonus / commission", "EPF employee", "SOCSO", "EIS", "PCB / tax", "Other deductions", "Employer contributions"].map((item) => <div key={item} className="flex items-center gap-2 rounded-xl bg-background px-3 py-2.5"><FileText className="h-3.5 w-3.5 text-accent" /><span>{item}</span></div>)}
        </div>
      </CardContent></Card>

      <div className="overflow-x-auto rounded-2xl border bg-well p-4 sm:p-6">
        <div className="mx-auto min-h-[820px] w-[680px] bg-card px-12 py-10 text-foreground shadow-xl">
          ...
          <div className="mt-7 grid grid-cols-2 gap-8 rounded-xl bg-background p-5 text-xs">
          ...
          <div className="mt-9 rounded-xl bg-foreground p-6 text-white"><div className="flex items-end justify-between"><div><div className="text-[10px] uppercase tracking-[.18em] text-white/50">Net salary</div><div className="mt-2 text-sm text-white/60">Gross pay less employee deductions</div></div><div className="text-3xl font-bold">{money(gross - deductions)}</div></div></div>
