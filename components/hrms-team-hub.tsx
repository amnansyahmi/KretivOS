      ...
      <Card className="border-black/8 bg-white/90">
        <CardContent className="p-5">
          ...
          {canManage && (events || []).length > 0 && <div className="mt-5 border-t pt-4">
            ...
            <div className="space-y-2">{(events || []).filter((item: any) => item.startDate?.slice(0, 7) === month).map((item: any) => <div key={item.id} className="flex items-center gap-3 rounded-xl bg-[#f7f4ed] p-3"><div className="min-w-0 flex-1"><div className="truncate text-xs font-semibold">{item.title}</div><div className="mt-1 truncate text-[9px] text-muted-foreground">{dateLabel(item.startDate)}{item.location ? ` · ${item.location}` : ""} · {item.status}</div></div><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onEditEvent(item)}><Pencil className="h-3.5 w-3.5" /></Button><Button variant="ghost" size="icon" className="h-8 w-8 text-red-500" onClick={() => onDeleteEvent(item.id)}><Trash2 className="h-3.5 w-3.5" /></Button></div>)}</div>
          </div>}
        </CardContent>
      </Card>
    </div>
