...
function SidebarContent({
  ...
  return <div className="flex h-full min-h-0 flex-col bg-foreground text-white">
    <div className="flex h-[88px] shrink-0 items-center gap-3 border-b border-white/10 px-5">
      <div className="flex h-10 w-10 items-center justify-center border border-white/15 bg-white/10 text-accent-muted"><BriefcaseBusiness className="h-5 w-5" /></div>
    ...
    <div className="shrink-0 border-b border-white/10 p-3">
      ...
      <button type="button" onClick={() => act(onCreateQuotation)} className="flex min-h-11 w-full items-center gap-3 bg-accent-soft px-3 text-left text-xs font-semibold text-foreground transition hover:bg-accent-muted">
      ...
      <div className="min-w-0 pb-24">
        <header className="sticky top-0 z-40 border-b border-black/5 bg-background/95 backdrop-blur-xl">
          <div className="mx-auto flex min-h-[72px] max-w-[1600px] items-center gap-3 px-4 md:min-h-24 md:px-8 md:py-5">
            <Button variant="outline" size="icon" className="shrink-0 bg-card lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Open sales navigation"><Menu className="h-5 w-5" /></Button>
            <div className="min-w-0 flex-1">
              <div className="hidden text-[10px] font-semibold uppercase tracking-[.2em] text-accent sm:block">Kretivco sales</div>
