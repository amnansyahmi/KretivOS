  ...
  return <div>
    ...
    <div className="grid gap-5 xl:grid-cols-[.78fr_1.22fr]">
      <Card className="bg-white/80"><CardHeader className="border-b"><CardTitle className="text-lg">Content brief</CardTitle></CardHeader><CardContent className="space-y-5 p-5">
        ...
        <label className="flex cursor-pointer items-start gap-3 rounded-xl border bg-card p-3"><input type="checkbox" checked={form.useCurrentResearch} onChange={(event) => set("useCurrentResearch", event.target.checked)} className="mt-1" /><span><span className="block text-sm font-medium">Use current web context</span><span className="mt-1 block text-[11px] leading-5 text-muted-foreground">Slower deep mode. Useful for current topics; external facts are used for framing, not added as unverified brand claims.</span></span></label>
      ...
      <Card className="overflow-hidden bg-white/85"><CardHeader className="border-b bg-foreground text-white"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><div className="text-[10px] font-semibold uppercase tracking-[.18em] text-accent-muted">Writing desk</div><CardTitle className="mt-2">{result ? result.summary : "Your content will appear here"}</CardTitle></div>{result && <span className={cn("w-fit rounded-full px-2.5 py-1 text-[10px]", result.source === "ai-nonymauz-cloud" ? "bg-emerald-400/15 text-emerald-200" : "bg-amber-400/15 text-amber-100")}>{result.source}</span>}</div></CardHeader><CardContent className="p-4 sm:p-5">
        {!result && !loading && <div className="flex min-h-[560px] flex-col items-center justify-center rounded-2xl border border-dashed p-8 text-center"><MessageSquareText className="h-8 w-8 text-accent" /><div className="mt-4 text-lg font-semibold">Start with a simple topic</div><p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">Content Writer will use the selected client, Brand DNA, platform behaviour and goal to produce editable copy—not a generic template.</p></div>}
        {loading && !result && <div className="flex min-h-[560px] flex-col items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-accent" /><div className="mt-4 text-sm font-medium">Building three different directions…</div><p className="mt-2 text-xs text-muted-foreground">Grounding the draft in your selected context.</p></div>}
        {result && activeVariant && <div>
          <div className="mb-4 flex gap-2 overflow-x-auto pb-1">{result.variants.map((variant, index) => <button key={`${variant.label}-${index}`} onClick={() => setActive(index)} className={cn("shrink-0 rounded-lg border px-3 py-2 text-xs font-medium", active === index ? "border-foreground bg-foreground text-white" : "bg-card text-muted-foreground hover:bg-background")}>{index + 1}. {variant.label}</button>)}</div>
          <div className={cn("space-y-4 transition-opacity", loading && "pointer-events-none opacity-55")}>
            ...
            {activeVariant.notes && <div className="rounded-xl border bg-background p-4 text-xs leading-6 text-muted-foreground"><span className="font-semibold text-foreground">Why this angle:</span> {activeVariant.notes}</div>}
            ...
            ].map(([label, prompt]) => <Button key={label} variant="outline" size="sm" className="bg-card" onClick={() => void requestWriter("refine", prompt)} disabled={loading}>{loading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <WandSparkles className="h-3.5 w-3.5" />}{label}</Button>)}</div></div>

            <div className="flex items-end gap-2 rounded-2xl border bg-card p-2"><Textarea value={instruction} onChange={(event) => setInstruction(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); submitInstruction(); } }} className="min-h-16 flex-1 resize-none border-0 bg-transparent focus-visible:ring-0" placeholder="Ask AI to revise this version…" /><Button size="icon" onClick={submitInstruction} disabled={!instruction.trim() || loading} aria-label="Send revision instruction"><Send className="h-4 w-4" /></Button></div>

            <div className="flex flex-col gap-3 rounded-2xl border bg-card p-4 sm:flex-row sm:items-end"><div className="min-w-0 flex-1"><div className="mb-2 text-xs font-medium">Send to planner</div><DateInput value={scheduleDate} onChange={(event) => { setScheduleDate(event.target.value); setScheduleState("idle"); }} /></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={copyDraft}>{copied ? <Check className="h-4 w-4 text-emerald-600" /> : <Clipboard className="h-4 w-4" />}{copied ? "Copied" : "Copy"}</Button><Button onClick={addToPlanner} disabled={!scheduleDate || scheduleState === "saving"}>{scheduleState === "saving" ? <Loader2 className="h-4 w-4 animate-spin" /> : scheduleState === "saved" ? <Check className="h-4 w-4" /> : <CalendarPlus className="h-4 w-4" />}{scheduleState === "saved" ? "Added" : "Add to planner"}</Button></div></div>
...
function Field({ label, required = false, children }: { label: string; required?: boolean; children: React.ReactNode }) {
  return <label className="block text-xs font-medium text-foreground-soft"><span>{label}{required && <span className="ml-1 text-accent">*</span>}</span><div className="mt-2">{children}</div></label>;
}
