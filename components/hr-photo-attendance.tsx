...
export function HRPhotoAttendance({ employees, attendance, query = "", onRefresh, onNotice, onError }: Props) {
  ...
  return <div className="space-y-5">
    <Card className="overflow-hidden bg-foreground text-white"><CardContent className="p-0"><div className="grid lg:grid-cols-2"><div className="p-5 sm:p-7"><div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.18em] text-accent-faint"><Camera className="h-4 w-4" />Photo attendance</div><h2 className="mt-3 text-2xl font-semibold">Clock in or out with photo and GPS</h2><p className="mt-2 text-sm text-white/55">Photo, official time and GPS evidence are stored separately.</p><div className="mt-6 grid grid-cols-3 gap-3"><StatDark label="Clocked in" value={`${todayRows.filter((row) => row.checkIn).length}/${active.length}`} /><StatDark label="Completed" value={todayRows.filter((row) => row.checkOut).length} /><StatDark label="Late" value={todayRows.filter((row) => Number(row.lateMinutes || 0) > 0).length} /></div></div><div className="border-t border-white/10 bg-black/10 p-5 sm:p-7 lg:border-l lg:border-t-0"><label className="text-xs text-white/70">Team member</label><select value={employeeId} onChange={(event) => setEmployeeId(event.target.value)} className="mt-2 h-12 w-full rounded-xl border border-white/15 bg-white/10 px-3 text-sm text-white">{active.map((employee) => <option key={employee.id} value={employee.id} className="text-black">{employee.name}</option>)}</select>{selected && <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-4"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-card text-xs font-bold text-foreground">{initials(selected.name)}</div><div><div className="font-semibold">{selected.name}</div><div className="text-[11px] text-white/45">{selected.title || "Kretivco team"}</div></div></div><div className="mt-4 grid grid-cols-2 gap-2"><MiniDark label="Clock in" value={todayRecord?.checkIn || "Not yet"} /><MiniDark label="Clock out" value={todayRecord?.checkOut || "Not yet"} /></div>{next ? <Button className="mt-4 h-12 w-full bg-accent-soft" onClick={() => setCamera({ employee: selected, action: next })}>{next === "check_in" ? <LogIn className="h-4 w-4" /> : <LogOut className="h-4 w-4" />}Take photo & {next === "check_in" ? "clock in" : "clock out"}</Button> : <div className="mt-4 flex h-12 items-center justify-center gap-2 rounded-xl bg-emerald-500/15 text-sm font-semibold text-emerald-200"><CheckCircle2 className="h-4 w-4" />Completed today</div>}</div>}</div></div></CardContent></Card>
...
export function HRAttendanceCapture({ employee, action, onClose, onDone, onError }: { employee: Employee; action: Action; onClose: () => void; onDone: (result: any) => Promise<void> | void; onError: (message: string) => void }) {
  ...
  return <div className="fixed inset-0 z-[170] flex items-end justify-center bg-black/70 sm:items-center sm:p-4"><div className="max-h-[96dvh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-foreground text-white sm:rounded-3xl"><div className="sticky top-0 z-20 flex justify-between border-b border-white/10 bg-foreground/95 p-4"><div><div className="text-[10px] font-semibold uppercase tracking-[.18em] text-accent-muted">{action === "check_in" ? "Clock in" : "Clock out"}</div><h2 className="mt-1 text-xl font-semibold">{employee.name}</h2></div><button onClick={onClose}><X className="h-5 w-5" /></button></div><div className="p-4 sm:p-5"><div className="relative aspect-[3/4] max-h-[42dvh] overflow-hidden rounded-2xl bg-black sm:max-h-[52dvh]"><video ref={video} muted playsInline className="h-full w-full scale-x-[-1] object-cover" /><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-4 pt-16"><div className="font-semibold">{employee.name}</div><div className="mt-1 font-mono text-xs text-white/70">{now.toLocaleString("en-MY", { timeZone: "Asia/Kuala_Lumpur", hour12: false })}</div><div className="mt-1 flex items-center gap-1 text-[10px] text-white/60"><MapPin className="h-3 w-3" />{geoBusy ? "Detecting location…" : geoLabel(geo)}</div></div>{error && <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90 p-6 text-center"><AlertTriangle className="h-8 w-8 text-amber-400" /><p className="mt-3 text-xs text-white/60">{error}</p><label className="mt-4 inline-flex h-11 cursor-pointer items-center gap-2 rounded-xl bg-card px-4 text-sm font-semibold text-black"><Camera className="h-4 w-4" />Camera fallback<input type="file" accept="image/*" capture="user" className="hidden" onChange={fallback} /></label></div>}</div><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-xs text-white/65">Work location<select value={workMode} onChange={(event) => setWorkMode(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-white/12 bg-white/8 px-3"><option className="text-black">Office</option><option className="text-black">Remote</option><option className="text-black">Client Site</option></select></label><label className="text-xs text-white/65">Note<input value={note} onChange={(event) => setNote(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-white/12 bg-white/8 px-3" placeholder="Optional" /></label></div><div className="mt-4 hidden items-start gap-3 rounded-xl border border-white/10 bg-white/5 p-3 text-[11px] text-white/55 sm:flex"><ShieldCheck className="h-4 w-4 shrink-0" />GPS and photo changes never modify official attendance time.</div><div className="mt-4 grid grid-cols-[auto_1fr] gap-2"><Button variant="outline" size="icon" className="h-12 border-white/15 bg-white/5 text-white" onClick={startCamera} aria-label="Reset"><RotateCcw className="h-4 w-4" /></Button><Button className="h-12 bg-accent-soft" disabled={busy || Boolean(error)} onClick={capture}>{busy ? <><RefreshCw className="h-4 w-4 animate-spin" />{geoBusy ? "Detecting GPS…" : "Submitting…"}</> : <><Camera className="h-4 w-4" />Capture & {action === "check_in" ? "clock in" : "clock out"}</>}</Button></div></div></div></div>;
}
