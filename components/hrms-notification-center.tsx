...
export function HRMSNotificationCenter() {
  ...
  return <>
    <Button variant="outline" size="icon" className="relative bg-white" onClick={() => setOpen(true)} aria-label={`${unread} unread HR notifications`}>
      <Bell className="h-4 w-4" />
      {unread > 0 && <span className="absolute -right-1.5 -top-1.5 flex h-5 min-w-5 items-center justify-center rounded-full bg-[#ba5c42] px-1 text-[9px] font-bold text-white">{unread > 99 ? "99+" : unread}</span>}
    </Button>

    {open && <div className="fixed inset-0 z-[190] bg-black/45" onClick={() => setOpen(false)}>
      <aside className="absolute inset-y-0 right-0 flex w-full max-w-md flex-col bg-[#f7f4ed] shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-start gap-3 border-b bg-white p-5"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#202c25] text-white"><Bell className="h-4 w-4" /></div><div className="min-w-0 flex-1"><h2 className="font-semibold">HR notifications</h2><p className="mt-1 text-xs text-muted-foreground">{unread} unread · updates every minute</p></div><Button variant="ghost" size="icon" onClick={() => setOpen(false)} aria-label="Close notifications"><X className="h-4 w-4" /></Button></div>
        ...
        {items.some((item) => item.read) && <div className="border-t bg-white p-4"><Button variant="outline" className="w-full" onClick={() => update("archive_read")}>Clear read notifications</Button></div>}
      </aside>
    </div>}
  </>;
}
