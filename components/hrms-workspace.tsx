  ...
  return (
    ...
    >
        ...
        {loading && !data.employees.length ? <div className="rounded-2xl border bg-white p-12 text-center text-sm text-muted-foreground">Loading HR workspace…</div> : (
          <>
            {!(["self", "overview", "payslips", "settings"] as Tab[]).includes(tab) && <div className="kretivos-search-control mb-4 flex h-11 items-center gap-2 rounded-xl border bg-white px-3"><Search className="h-5 w-5 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm outline-none" placeholder={`Search ${tabs.find((item) => item.id === tab)?.label.toLowerCase()}...`} /></div>}
...
function HRMSSettings({ settings, saving, authEnabled, onSave }: { settings: Snapshot["settings"]; saving: boolean; authEnabled: boolean; onSave: (settings: Snapshot["settings"]) => void }) {
  ...
  return <div className="space-y-5">
    <Card className="border-black/8 bg-white/90"><CardContent className="p-5"><div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#202c25] text-white"><Settings2 className="h-4 w-4" /></div><div><h2 className="font-semibold">HR master lists</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">One item per line. Changes are used immediately by employee and leave forms.</p></div></div><div className="mt-5 grid gap-4 lg:grid-cols-3"><label className="text-xs font-medium text-[#4e5a52]">Departments<Textarea value={lists.departments} onChange={(event) => setLists({ ...lists, departments: event.target.value })} className="mt-2 min-h-44" /></label><label className="text-xs font-medium text-[#4e5a52]">Leave types<Textarea value={lists.leaveTypes} onChange={(event) => setLists({ ...lists, leaveTypes: event.target.value })} className="mt-2 min-h-44" /></label><label className="text-xs font-medium text-[#4e5a52]">Work modes<Textarea value={lists.workModes} onChange={(event) => setLists({ ...lists, workModes: event.target.value })} className="mt-2 min-h-44" /></label></div></CardContent></Card>
...
function Setting({ label, wide, children }: { label: string; wide?: boolean; children: React.ReactNode }) { return <Label className={cn("text-[#4e5a52]", wide && "sm:col-span-2")}>{label}<div className="mt-2">{children}</div></Label>; }
  ...
  return <div className="fixed inset-0 z-[150] flex items-end justify-center bg-black/45 sm:items-center sm:p-4">
    <Card className="max-h-[94dvh] w-full max-w-3xl overflow-y-auto rounded-b-none bg-[#f7f4ed] shadow-2xl sm:rounded-2xl">
      ...
      <CardContent className="space-y-4 p-4 sm:p-6">
        ...
        {editor.resource === "announcements" && <div className="grid gap-4 sm:grid-cols-2"><Field label="Announcement title" wide><Input value={record.title || ""} onChange={(e) => update("title", e.target.value)} /></Field><Field label="Category"><Select value={record.category || "General"} onChange={(e) => update("category", e.target.value)} >{["General", "People", "Operations", "Policy", "Event", "Urgent"].map((item) => <option key={item}>{item}</option>)}</Select></Field><Field label="Status"><Select value={record.status || "Published"} onChange={(e) => update("status", e.target.value)} ><option>Draft</option><option>Published</option><option>Archived</option></Select></Field><Field label="Publish date"><DateInput value={record.publishAt || ""} onChange={(e) => update("publishAt", e.target.value)} /></Field><Field label="Expiry date"><DateInput value={record.expiresAt || ""} onChange={(e) => update("expiresAt", e.target.value)} /></Field><label className="flex items-center gap-3 rounded-xl border bg-white p-3 text-xs font-medium text-[#4e5a52]"><input type="checkbox" checked={Boolean(record.pinned)} onChange={(e) => update("pinned", e.target.checked)} className="h-4 w-4" />Pin this announcement</label><Field label="Announcement" wide><Textarea value={record.body || ""} onChange={(e) => update("body", e.target.value)} className="min-h-40" /></Field></div>}
