    ...
    >
        ...
        {loading && !data.employees.length ? <div className="rounded-2xl border bg-card p-12 text-center text-sm text-muted-foreground">Loading HR workspace…</div> : (
          <>
            ...
            {tab === "self" && <HRSelfService session={session} employee={data.employees.find((item) => item.id === session.userId)} leave={data.leaveRequests.filter((item) => item.employeeId === session.userId)} attendance={data.attendance.filter((item) => item.employeeId === session.userId)} claims={data.claims.filter((item) => item.employeeId === session.userId)} payroll={data.payroll.filter((item) => item.employeeId === session.userId)} timesheets={data.timesheets} documents={data.documents} publicHolidays={data.settings.publicHolidays || []} settings={data.settings} onEdit={(employee: any) => openEdit("employees", employee)} onCreate={(resource: Resource) => openCreate(resource, session.userId)} onNavigate={changeTab} onCompleteStep={(stepId: string) => completeStep(session.userId, stepId)} />}
  ...
  return <div className="fixed inset-0 z-[150] flex items-end justify-center bg-black/45 sm:items-center sm:p-4">
    <Card className="max-h-[94dvh] w-full max-w-3xl overflow-y-auto rounded-b-none bg-background shadow-2xl sm:rounded-2xl">
      ...
      <CardContent className="space-y-4 p-4 sm:p-6">
        ...
        {editor.resource === "attendance" && <div className="grid gap-4 sm:grid-cols-2">{employeeSelect}<Field label="Date"><DateInput value={record.date || ""} onChange={(e) => update("date", e.target.value)} /></Field><Field label="Status"><Select value={record.status || "Present"} onChange={(e) => update("status", e.target.value)} >{["Present", "WFH", "Leave", "Absent", "Off", "Client Site"].map((item) => <option key={item}>{item}</option>)}</Select></Field><Field label="Check in"><TimeInput value={record.checkIn || ""} onChange={(e) => update("checkIn", e.target.value)} /></Field><Field label="Check out"><TimeInput value={record.checkOut || ""} onChange={(e) => update("checkOut", e.target.value)} /></Field><Field label="Note" wide><Textarea value={record.note || ""} onChange={(e) => update("note", e.target.value)} /></Field></div>}
