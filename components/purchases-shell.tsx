...
function SidebarContent({
  ...
  return <div className="flex h-full min-h-0 flex-col bg-[#1c2b23] text-white">
    <div className="flex h-[88px] shrink-0 items-center gap-3 border-b border-white/10 px-5">
      <div className="flex h-10 w-10 items-center justify-center border border-white/15 bg-white/10 text-[#f19a7f]"><ShoppingCart className="h-5 w-5" /></div>
    ...
    <div className="shrink-0 border-b border-white/10 p-3">
      ...
      <button type="button" onClick={() => act(onNewExpense)} className="flex min-h-11 w-full items-center gap-3 bg-[#ef8a6b] px-3 text-left text-xs font-semibold text-[#1c2b23] transition hover:bg-[#f19a7f]">
      ...
      <div className="min-w-0 pb-24">
        <header className="sticky top-0 z-40 border-b border-black/5 bg-[#f5f2ea]/95 backdrop-blur-xl">
          <div className="mx-auto flex min-h-[72px] max-w-[1600px] items-center gap-3 px-4 md:min-h-24 md:px-8 md:py-5">
            <Button variant="outline" size="icon" className="shrink-0 bg-white lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Open purchases navigation"><Menu className="h-5 w-5" /></Button>
            <div className="min-w-0 flex-1">
              <div className="hidden text-[10px] font-semibold uppercase tracking-[.2em] text-[#ba5c42] sm:block">Kretivco purchases</div>
