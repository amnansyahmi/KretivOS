    ...
    >
      ...
      {tab === "workflows" && <div className="grid gap-5 xl:grid-cols-[370px_minmax(0,1fr)]">
        <Card className={cn("h-fit bg-white/80 xl:sticky xl:top-5 xl:block", !showList && "hidden")}>
          <CardHeader className="border-b p-4 sm:p-5"><div className="flex items-center justify-between"><div><CardTitle>Workflow library</CardTitle><p className="mt-1 text-xs text-muted-foreground">{recipes.filter((item) => item.status === "Active").length} active</p></div><Workflow className="h-5 w-5 text-[#ba5c42]" /></div><div className="mt-4 flex h-10 items-center gap-2 rounded-lg border bg-white px-3"><Search className="h-4 w-4 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="min-w-0 flex-1 text-sm outline-none" placeholder="Search workflows" /></div></CardHeader>
