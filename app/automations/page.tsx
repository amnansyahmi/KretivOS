...
export default function AutomationsPage() {
  ...
  async function deleteRecipe() {
    if (!selected || !await confirm({ title: `Delete “${selected.name}”?`, description: "The recipe stops running immediately.", destructive: true })) return;
    try {
  ...
  return (
    <WorkspacePage
      ...
      actions={<div className="flex flex-wrap gap-2"><Button variant="outline" className="bg-card" onClick={() => void refresh(true)} disabled={loading}><Activity className={cn("h-4 w-4", loading && "animate-pulse")} />Sync server</Button><Button variant="outline" className="bg-card" asChild><Link href="/approvals"><ShieldCheck className="h-4 w-4" />Approval Inbox{pendingApprovals.length ? ` (${pendingApprovals.length})` : ""}</Link></Button><button onClick={() => setShowList((value) => !value)} className="inline-flex h-10 items-center gap-2 rounded-lg border bg-card px-4 text-sm font-medium xl:hidden"><List className="h-4 w-4" />{showList ? "Hide list" : "Show list"}</button><Button onClick={openCreate}><Plus className="h-4 w-4" />New automation</Button></div>}
    >
      ...
      <div className="mb-5 overflow-x-auto rounded-xl border bg-white/70 p-1.5">
        <div className="flex min-w-max gap-1">{tabs.map(({ id, label, count, icon: Icon }) => <button key={id} onClick={() => setTab(id)} className={cn("flex min-h-11 items-center gap-2 rounded-lg px-4 text-sm font-medium transition", tab === id ? "bg-foreground text-white" : "text-muted-foreground hover:bg-background")}><Icon className="h-4 w-4" />{label}{typeof count === "number" && <span className={cn("rounded-full px-2 py-0.5 text-[10px]", tab === id ? "bg-white/15" : "bg-muted")}>{count}</span>}</button>)}</div>
      </div>
      ...
      {tab === "workflows" && <div className="grid gap-5 xl:grid-cols-[370px_minmax(0,1fr)]">
        <Card className={cn("h-fit bg-white/80 xl:sticky xl:top-5 xl:block", !showList && "hidden")}>
          <CardHeader className="border-b p-4 sm:p-5"><div className="flex items-center justify-between"><div><CardTitle>Workflow library</CardTitle><p className="mt-1 text-xs text-muted-foreground">{recipes.filter((item) => item.status === "Active").length} active</p></div><Workflow className="h-5 w-5 text-accent" /></div><div className="kretivos-search-control mt-4 flex h-10 items-center gap-2 rounded-xl border bg-card px-3"><Search className="h-5 w-5 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="min-w-0 flex-1 text-sm outline-none" placeholder="Search workflows" /></div></CardHeader>
          <CardContent className="max-h-[58vh] space-y-2 overflow-y-auto p-3 xl:max-h-[calc(100vh-250px)]">{filtered.map((item) => <button key={item.id} onClick={() => { setSelectedId(item.id); setShowList(false); }} className={cn("w-full rounded-xl border p-4 text-left", selected?.id === item.id ? "border-accent bg-card" : "bg-card hover:bg-background")}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate text-[10px] font-semibold uppercase tracking-wider text-accent">{item.workspace}</div><div className="mt-1 text-sm font-semibold">{item.name}</div></div><ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground" /></div><p className="mt-2 line-clamp-2 text-xs leading-5 text-muted-foreground">{item.description}</p><div className="mt-3 flex flex-wrap items-center gap-2"><span className={cn("rounded-full px-2.5 py-1 text-[10px] font-medium", statusTone(item.status))}>{item.status}</span><span className="text-[10px] text-muted-foreground">{item.actions.length} actions</span>{item.approvalRequired && <span className="text-[10px] text-amber-700">Approval</span>}</div></button>)}</CardContent>
        </Card>

        <div className="min-w-0 space-y-5">
          {selected ? <>
            <Card className="overflow-hidden bg-foreground text-white"><CardContent className="p-5 sm:p-7"><div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-start"><div><div className="text-[10px] uppercase tracking-[.18em] text-white/45">{selected.workspace} · {selected.system ? "KretivOS recipe" : "Custom recipe"}</div><h2 className="mt-3 text-2xl font-semibold sm:text-3xl">{selected.name}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-white/65">{selected.description}</p><div className="mt-4 inline-flex items-center gap-2 rounded-full bg-white/8 px-3 py-1.5 text-xs text-white/75"><Zap className="h-3.5 w-3.5 text-accent-muted" />When: {triggerLabel(selected.trigger)}</div></div><div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" className="border-white/15 bg-white/5 text-white hover:bg-white/10" onClick={toggleStatus}>{selected.status === "Active" ? <><Pause className="h-3.5 w-3.5" />Pause</> : <><Play className="h-3.5 w-3.5" />Activate</>}</Button><Button variant="outline" size="sm" className="border-white/15 bg-white/5 text-white hover:bg-white/10" onClick={openEdit}><Pencil className="h-3.5 w-3.5" />Edit</Button><Button variant="outline" size="sm" className="border-red-300/20 bg-red-500/10 text-red-100 hover:bg-red-500/20" onClick={deleteRecipe}><Trash2 className="h-3.5 w-3.5" />Delete</Button></div></div><div className="mt-6 grid gap-3 sm:grid-cols-3"><Metric label="Executions" value={String(selected.runCount || 0)} /><Metric label="Last run" value={formatDate(selected.lastRun)} /><Metric label="Execution mode" value={selected.approvalRequired ? "Approval required" : "Automatic"} /></div><Button className="mt-6 w-full bg-accent-soft hover:bg-accent sm:w-auto" onClick={runNow}><Zap className="h-4 w-4" />Run with latest record</Button></CardContent></Card>

            <Card className="bg-white/80"><CardHeader><CardTitle>Action sequence</CardTitle><p className="mt-1 text-xs text-muted-foreground">Every action is idempotent. Running it again will skip records that already exist.</p></CardHeader><CardContent className="space-y-3">{selected.actions.map((action, index) => <div key={action} className="flex items-center gap-3 rounded-xl border bg-card p-4"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-foreground text-xs text-white">{index + 1}</span><div className="min-w-0 flex-1"><div className="text-sm font-medium">{actionLabel(action)}</div><div className="mt-1 text-xs text-muted-foreground">{ACTION_CATALOG.find((item) => item.id === action)?.description}</div></div>{index < selected.actions.length - 1 && <ChevronRight className="h-4 w-4 text-muted-foreground" />}</div>)}</CardContent></Card>

            <Card className="bg-white/80"><CardHeader><div className="flex items-center justify-between"><div><CardTitle>Recent executions</CardTitle><p className="mt-1 text-xs text-muted-foreground">Latest runs for this workflow</p></div><Activity className="h-5 w-5 text-accent" /></div></CardHeader><CardContent className="space-y-2">{selectedRuns.map((run) => <RunRow key={run.id} run={run} />)}{!selectedRuns.length && <Empty icon={Clock} title="No execution yet" description="Run it manually or wait for its trigger." />}</CardContent></Card>
      ...
      {tab === "notifications" && <div className="space-y-4"><div className="flex justify-end"><Button variant="outline" onClick={clearReadNotifications} disabled={!notifications.some((item) => item.read)}>Clear read</Button></div>{notifications.map((notification) => <Card key={notification.id} className={cn("bg-white/80", !notification.read && "border-accent/40")}><CardContent className="flex flex-col justify-between gap-4 p-5 sm:flex-row sm:items-center"><div className="flex gap-3"><div className={cn("flex h-10 w-10 shrink-0 items-center justify-center rounded-xl", notification.type === "Warning" ? "bg-amber-50 text-amber-700" : "bg-muted text-accent")}>{notification.type === "Warning" ? <CircleAlert className="h-5 w-5" /> : <Bell className="h-5 w-5" />}</div><div><h3 className="text-sm font-semibold">{notification.title}</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">{notification.description}</p><p className="mt-2 text-[10px] text-muted-foreground">{formatDate(notification.createdAt)}</p></div></div><div className="flex gap-2">{notification.href && <a href={notification.href} className="inline-flex h-10 items-center rounded-lg bg-foreground px-4 text-sm font-medium text-white">Open record</a>}{!notification.read && <Button variant="outline" onClick={() => markNotificationRead(notification.id)}>Mark read</Button>}</div></CardContent></Card>)}{!notifications.length && <EmptyCard icon={Bell} title="No notifications" description="The daily scan will surface records that require attention." />}</div>}
      ...
      {editorOpen && <div className="fixed inset-0 z-[150] flex items-end justify-center bg-black/45 sm:items-center sm:p-4"><Card className="max-h-[96vh] w-full max-w-3xl overflow-y-auto rounded-b-none bg-background shadow-2xl sm:rounded-xl"><CardHeader className="sticky top-0 z-10 flex-row items-start justify-between border-b bg-background p-4 sm:p-6"><div><CardTitle className="text-xl">{draft.id ? "Edit automation" : "Create automation"}</CardTitle><p className="mt-1 text-xs text-muted-foreground">Choose a supported trigger and one or more compatible actions.</p></div><Button variant="ghost" size="icon" aria-label="Close editor" onClick={() => setEditorOpen(false)}><X className="h-4 w-4" /></Button></CardHeader><CardContent className="p-4 sm:p-6"><div className="grid gap-4 sm:grid-cols-2"><Field label="Automation name" wide><input value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} className="input" placeholder="Example: New customer setup" /></Field><Field label="Workspace"><input value={draft.workspace} onChange={(event) => setDraft({ ...draft, workspace: event.target.value })} className="input" /></Field><Field label="Trigger"><select value={draft.trigger} onChange={(event) => changeTrigger(event.target.value as AutomationTrigger)} className="input">{TRIGGER_CATALOG.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></Field><Field label="Status"><select value={draft.status} onChange={(event) => setDraft({ ...draft, status: event.target.value as AutomationRecipe["status"] })} className="input"><option>Active</option><option>Paused</option></select></Field><Field label="Description" wide><Textarea value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} placeholder="Explain the business outcome of this automation." /></Field></div><div className="mt-5"><div className="text-xs font-medium">Actions</div><p className="mt-1 text-[11px] text-muted-foreground">Only actions compatible with {triggerLabel(draft.trigger)} are shown.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{compatibleActions.map((action) => <label key={action.id} className={cn("flex cursor-pointer gap-3 rounded-xl border p-4 transition", draft.actions.includes(action.id) ? "border-accent bg-card" : "bg-card hover:bg-card")}><input type="checkbox" checked={draft.actions.includes(action.id)} onChange={() => toggleAction(action.id)} className="mt-0.5 h-4 w-4 accent-accent" /><span><span className="block text-sm font-medium">{action.label}</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">{action.description}</span></span></label>)}</div></div><label className="mt-5 flex items-start gap-3 rounded-xl border bg-card p-4"><input type="checkbox" checked={draft.approvalRequired} onChange={(event) => setDraft({ ...draft, approvalRequired: event.target.checked })} className="mt-0.5 h-4 w-4 accent-accent" /><span><span className="block text-sm font-medium">Require human approval</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">Use this for actions that create commercial commitments or delivery records.</span></span></label><div className="mt-6 flex flex-col-reverse gap-2 border-t pt-5 sm:flex-row sm:justify-end"><Button variant="outline" onClick={() => setEditorOpen(false)}>Cancel</Button><Button onClick={saveRecipe}><Save className="h-4 w-4" />Save automation</Button></div></CardContent></Card></div>}
    </WorkspacePage>
  );
}
...
function RunRow({ run }: { run: AutomationRun }) {
  return <div className="flex flex-col justify-between gap-3 rounded-xl border bg-card p-4 sm:flex-row sm:items-center"><div><div className="flex flex-wrap items-center gap-2"><span className={cn("rounded-full px-2.5 py-1 text-[10px] font-medium", statusTone(run.status))}>{run.status}</span><span className="text-xs font-medium">{run.recipeName}</span></div><div className="mt-2 text-xs text-muted-foreground">{run.summary || triggerLabel(run.trigger)}</div>{run.error && <div className="mt-1 text-xs text-red-600">{run.error}</div>}</div><div className="text-[10px] text-muted-foreground sm:text-right"><div>{formatDate(run.startedAt)}</div><div className="mt-1">{run.actions.length} actions</div></div></div>;
}
