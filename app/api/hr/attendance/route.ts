...
async function insertPhoto(params: {
  ...
  await sql`
    insert into assets (
      id, organization_id, name, asset_type, category, storage_url,
      mime_type, file_size, metadata
    ) values (
      ${id}, ${ORGANIZATION_ID},
      ${`Attendance ${params.action} ${params.employeeId} ${params.officialAt}`},
      'hr_attendance_photo', ${params.action}, ${params.photoDataUrl},
      ${params.mimeType}, ${params.fileSize},
      ${JSON.stringify({
        attendanceId: params.attendanceId,
        employeeId: params.employeeId,
        action: params.action,
        capturedAtDevice: params.capturedAtDevice,
        officialAt: params.officialAt,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const employees = await sql`
      ...
      where id = ${employeeId} and organization_id = ${ORGANIZATION_ID} and status = 'active'
      limit 1
    `;
    ...
    const verification = captureIsValid && driftSeconds <= MAX_CLOCK_DRIFT_SECONDS
      ? "device_capture_verified"
      : "server_adjusted";
    ...
    insertedPhotoId = await insertPhoto({
      attendanceId,
      employeeId,
      action,
      photoDataUrl: photo.dataUrl,
      mimeType: photo.mimeType,
      fileSize: photo.estimatedBytes,
      capturedAtDevice: capturedAtDevice.toISOString(),
      officialAt,
    ...
    let record: any;
    if (action === "check_in") {
      const clockMinutes = minutesFromClock(local.time);
      const lateAfter = minutesFromClock(clean(settings.shiftStart) || "09:00") + Number(settings.graceMinutes || 0);
      const lateMinutes = Math.max(0, clockMinutes - lateAfter);
      ...
      record = {
        ...
        workMode,
        checkIn: local.time,
        checkInAt: officialAt,
        checkInDeviceAt: capturedAtDevice.toISOString(),
        checkInServerAt: serverReceivedAt.toISOString(),
      ...
      const durationMinutes = Number.isFinite(checkInAt.getTime())
        ? Math.max(0, Math.round((officialDate.getTime() - checkInAt.getTime()) / 60_000))
        : 0;
      record = {
        ...existing,
        workMode: existing.workMode || workMode,
        checkOut: local.time,
        checkOutAt: officialAt,
        checkOutDeviceAt: capturedAtDevice.toISOString(),
        checkOutServerAt: serverReceivedAt.toISOString(),
    ...
    state.attendance = existing
      ? list.map((item: any) => item.id === attendanceId ? record : item)
      : [record, ...list];
    await saveOperations(state);
    await audit(`hr.attendance.photo_${action}`, attendanceId, {
      ...record,
      employeeName: employees[0].display_name,
      photoId: insertedPhotoId,
      timestampLabel: local.timestampLabel,
    });
    ...
    if (insertedPhotoId) {
      try {
        const sql = getDatabase();
        await sql`delete from assets where id = ${insertedPhotoId} and organization_id = ${ORGANIZATION_ID}`;
      } catch {}
    }
