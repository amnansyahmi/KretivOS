...
export default function BrandDNAWorkspace() {
  ...
  async function deleteProfile() {
    ...
    if (!window.confirm("Delete this draft Brand DNA version?")) return;
    setSaving(true);
    try {
  ...
  return (
    <WorkspacePage eyebrow="Multi-brand source of truth" title="Brand DNA" description="Create a separate, versioned identity for every customer brand. AI may suggest missing fields, but only human-reviewed values can be approved." actions={<div className="flex gap-2"><Button variant="outline" className="bg-white" onClick={loadData} disabled={loading}><RefreshCw className={cn("h-4 w-4", loading && "animate-spin")} /></Button>{selectedBrand && <Button onClick={createVersion}><Copy className="h-4 w-4" />New version</Button>}</div>}>
      ...
      <div className="grid gap-5 xl:grid-cols-[350px_minmax(0,1fr)]">
        <Card className={cn("overflow-hidden bg-white/80", mobileDetail && "hidden xl:block")}><CardHeader className="border-b p-4"><div className="flex items-center justify-between"><div><CardTitle>Customer brands</CardTitle><p className="mt-1 text-xs text-muted-foreground">{brands.length} brands</p></div><Palette className="h-5 w-5 text-[#ba5c42]" /></div><div className="mt-4 flex h-11 items-center gap-2 rounded-xl border bg-white px-3"><Search className="h-4 w-4 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="min-w-0 flex-1 text-sm outline-none" placeholder="Search customer or brand" /></div></CardHeader><CardContent className="max-h-[68vh] space-y-2 overflow-y-auto p-3">{filteredBrands.map((brand) => { const latest = brand.profiles[0]; return <button key={brand.id} onClick={() => selectBrand(brand.id)} className={cn("w-full rounded-2xl border p-4 text-left", selectedBrand?.id === brand.id ? "border-[#ba5c42] bg-[#fff8f4]" : "bg-white hover:bg-[#f8f5ee]")}><div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#eee9df]"><Palette className="h-4 w-4 text-[#ba5c42]" /></div><div className="min-w-0 flex-1"><div className="font-semibold">{brand.name}</div><div className="mt-1 text-[11px] text-muted-foreground">{brand.customerName}</div><div className="mt-3 flex items-center justify-between"><span className="text-[10px] text-muted-foreground">{brand.profiles.length} version{brand.profiles.length === 1 ? "" : "s"}</span><span className={cn("rounded-full px-2 py-1 text-[10px]", latest?.status === "Approved" ? "bg-emerald-50 text-emerald-700" : "bg-[#eeeae0] text-[#5a605a]")}>{latest?.status || "Not started"}</span></div></div></div></button>; })}{!filteredBrands.length && <div className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">No brands found. Add a brand in Business Workspace first.</div>}</CardContent></Card>

        <div className={cn("min-w-0", !mobileDetail && "hidden xl:block")}>
          {selectedBrand && draft ? <div className="space-y-5">
            <Card className="bg-white/80"><CardHeader className="border-b p-4 sm:p-6"><div className="flex items-start gap-3 xl:hidden"><button onClick={() => setMobileDetail(false)} className="flex h-9 w-9 items-center justify-center rounded-xl border bg-white"><ChevronLeft className="h-4 w-4" /></button><div className="min-w-0 flex-1"><CardTitle>{selectedBrand.name}</CardTitle><p className="mt-1 text-xs text-muted-foreground">{selectedBrand.customerName}</p></div></div><div className="hidden items-start justify-between gap-4 xl:flex"><div><div className="text-[10px] font-semibold uppercase tracking-[.15em] text-[#ba5c42]">{selectedBrand.customerName}</div><CardTitle className="mt-2 text-2xl">{selectedBrand.name}</CardTitle><p className="mt-2 text-sm text-muted-foreground">{selectedBrand.description || "No brand description yet."}</p></div><div className="text-right"><div className="text-3xl font-semibold">{draft.completenessScore}%</div><div className="text-[10px] uppercase tracking-wider text-muted-foreground">Completeness</div></div></div><div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><select value={selectedProfileId} onChange={(event) => setSelectedProfileId(event.target.value)} className="h-10 rounded-xl border bg-white px-3 text-sm"><option value="">Unsaved new draft</option>{selectedBrand.profiles.map((profile, index) => <option key={profile.id} value={profile.id}>Version {selectedBrand.profiles.length - index} · {profile.status} · {formatDate(profile.updatedAt)}</option>)}</select><div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={generateSuggestions} disabled={generating}><Bot className="h-3.5 w-3.5" />{generating ? "Analysing…" : "AI suggestions"}</Button><Button variant="outline" size="sm" onClick={() => saveProfile()} disabled={saving}><Save className="h-3.5 w-3.5" />Save draft</Button><Button size="sm" onClick={() => saveProfile("Approved")} disabled={saving || !draft.id}><ShieldCheck className="h-3.5 w-3.5" />Approve</Button>{draft.id && draft.status !== "Approved" && <Button variant="outline" size="icon" onClick={deleteProfile}><Trash2 className="h-4 w-4 text-red-500" /></Button>}</div></div></CardHeader><CardContent className="p-4 sm:p-6"><div className="grid gap-4 md:grid-cols-2"><Field label="Positioning" wide><textarea value={draft.positioning} onChange={(event) => setDraft({ ...draft, positioning: event.target.value })} className="textarea" placeholder="How the brand should be understood relative to alternatives" /></Field><Field label="Primary audience"><textarea value={draft.audience} onChange={(event) => setDraft({ ...draft, audience: event.target.value })} className="textarea" /></Field><Field label="Brand personality"><textarea value={draft.personality} onChange={(event) => setDraft({ ...draft, personality: event.target.value })} className="textarea" /></Field><Field label="Tone of voice" wide><textarea value={draft.voice} onChange={(event) => setDraft({ ...draft, voice: event.target.value })} className="textarea" /></Field><Field label="Value proposition" wide><textarea value={String(draft.messaging.valueProposition || "")} onChange={(event) => setDraft({ ...draft, messaging: { ...draft.messaging, valueProposition: event.target.value } })} className="textarea" /></Field><Field label="Core messages"><textarea value={String(draft.messaging.coreMessages || "")} onChange={(event) => setDraft({ ...draft, messaging: { ...draft.messaging, coreMessages: event.target.value } })} className="textarea" placeholder="One message per line" /></Field><Field label="Preferred words"><textarea value={String(draft.messaging.preferredWords || "")} onChange={(event) => setDraft({ ...draft, messaging: { ...draft.messaging, preferredWords: event.target.value } })} className="textarea" /></Field><Field label="Colour palette"><input value={draft.colours.join(", ")} onChange={(event) => setDraft({ ...draft, colours: parseLines(event.target.value) })} className="control" placeholder="#202C25, #EF7F5F" /><div className="mt-3 flex flex-wrap gap-2">{draft.colours.map((colour) => <div key={colour} className="flex items-center gap-2 rounded-lg border bg-white px-2 py-1.5 text-[10px]"><span className="h-5 w-5 rounded border" style={{ background: colour }} />{colour}</div>)}</div></Field><Field label="Heading typeface"><input value={String(draft.typography.heading || "")} onChange={(event) => setDraft({ ...draft, typography: { ...draft.typography, heading: event.target.value } })} className="control" /></Field><Field label="Body typeface"><input value={String(draft.typography.body || "")} onChange={(event) => setDraft({ ...draft, typography: { ...draft.typography, body: event.target.value } })} className="control" /></Field><Field label="Photography direction" wide><textarea value={draft.photographyDirection} onChange={(event) => setDraft({ ...draft, photographyDirection: event.target.value })} className="textarea min-h-28" /></Field><Field label="Approved factual claims"><textarea value={draft.approvedClaims.join("\n")} onChange={(event) => setDraft({ ...draft, approvedClaims: parseLines(event.target.value) })} className="textarea" placeholder="One verified claim per line" /></Field><Field label="Avoid / restricted language"><textarea value={draft.avoidList.join("\n")} onChange={(event) => setDraft({ ...draft, avoidList: parseLines(event.target.value) })} className="textarea" placeholder="Words, claims or visual directions to avoid" /></Field></div></CardContent></Card>

            <Card className="bg-white/80"><CardHeader className="border-b p-4"><div className="flex items-center justify-between"><div><CardTitle>Evidence</CardTitle><p className="mt-1 text-xs text-muted-foreground">Linked customer and brand knowledge used during review.</p></div><span className="rounded-full bg-[#eeeae0] px-2.5 py-1 text-[10px]">{linkedKnowledge.length} sources</span></div></CardHeader><CardContent className="grid gap-2 p-4 sm:grid-cols-2">{linkedKnowledge.map((entry) => <div key={entry.id} className="rounded-xl border bg-white p-3"><div className="text-sm font-medium">{entry.title}</div><div className="mt-1 text-[10px] text-muted-foreground">{entry.category}</div></div>)}{!linkedKnowledge.length && <div className="rounded-xl border border-dashed p-6 text-center text-xs text-muted-foreground sm:col-span-2">No linked knowledge yet. Add customer or brand knowledge first for stronger AI suggestions.</div>}</CardContent></Card>

            {draft.aiSuggestions.length > 0 && <Card className="border-[#ba5c42]/30 bg-[#fff8f4]"><CardHeader className="border-b border-[#ba5c42]/15 p-4"><div className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-[#ba5c42]" /><CardTitle>AI suggestions awaiting review</CardTitle></div><p className="mt-1 text-xs text-muted-foreground">Nothing is applied automatically. Review evidence and apply or reject each suggestion.</p></CardHeader><CardContent className="space-y-3 p-4">{draft.aiSuggestions.map((suggestion, index) => <div key={`${suggestion.field}-${index}`} className="rounded-xl border bg-white p-4"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div className="min-w-0"><div className="text-sm font-semibold">{suggestion.label || suggestion.field}</div><div className="mt-2 whitespace-pre-wrap text-xs leading-6 text-[#4e5a52]">{Array.isArray(suggestion.value) ? suggestion.value.join("\n") : String(suggestion.value ?? "")}</div>{suggestion.reason && <div className="mt-2 text-[10px] leading-5 text-muted-foreground">Why: {suggestion.reason}</div>}</div><div className="flex shrink-0 gap-2"><Button variant="outline" size="sm" onClick={() => rejectSuggestion(suggestion)}><X className="h-3.5 w-3.5" />Reject</Button><Button size="sm" onClick={() => applySuggestion(suggestion)}><Check className="h-3.5 w-3.5" />Apply</Button></div></div></div>)}</CardContent></Card>}
...
function Field({ label, wide, children }: { label: string; wide?: boolean; children: React.ReactNode }) {
  ...
  return <div className={cn("block text-xs font-medium text-[#4e5a52]", wide && "md:col-span-2")}><div className="flex min-h-8 items-center justify-between gap-2"><span>{label}</span>{canImprove && <AIWritingButton value={value} field={label} context="KretivOS Brand DNA. Preserve evidence, factual claims and the intended brand voice." onApply={(next) => control.props.onChange?.({ target: { value: next }, currentTarget: { value: next } })} />}</div><div className="mt-2">{children}</div></div>;
}
