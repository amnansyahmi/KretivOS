...
function MobileAgentCard({ agent, onOpen }: { agent: AgentView; onOpen: () => void }) {
  return (
    <button type="button" onClick={onOpen} className="min-w-0 overflow-hidden rounded-2xl border border-white/[.07] bg-[#151916] p-3 text-left active:scale-[.99]">
      <div className="flex min-w-0 items-start gap-2.5">
        <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-white/[.07] bg-black/25 text-lg">
          {agent.emoji}
          <span className={`absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full ring-2 ring-[#151916] ${statusDot(agent.status)}`} />
        </div>
        <div className="min-w-0 flex-1">
          <div className="truncate text-[12px] font-semibold text-white/90">{agent.name}</div>
          <div className="mt-0.5 truncate text-[8px] uppercase tracking-[.12em] text-white/25">{agent.department}</div>
        </div>
        <ChevronRight className="mt-1 h-3.5 w-3.5 shrink-0 text-white/20" />
      </div>
      <div className="mt-3 flex min-w-0 items-center gap-2">
        <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-black/25 px-2 py-1 text-[8px] text-white/42"><span className={`h-1.5 w-1.5 rounded-full ${statusDot(agent.status)}`} />{statusLabel(agent.status)}</span>
        <span className="min-w-0 flex-1 truncate text-[9px] text-white/30">{agent.detail}</span>
      </div>
    </button>
  );
}

function DesktopStation({ agent, selected, motion, onSelect }: { agent: AgentView; selected: boolean; motion: boolean; onSelect: () => void }) {
  const working = agent.status === "working";
  return (
    <button type="button" onClick={onSelect} className={`group min-w-0 rounded-2xl p-2 text-center transition ${selected ? "bg-white/[.05]" : "hover:bg-white/[.025]"}`}>
      <div className="relative mx-auto h-[108px] max-w-[148px]">
        <div className="absolute inset-x-3 bottom-2 h-[45px] rounded-[18px] bg-[#1a211b] shadow-[0_22px_30px_rgba(0,0,0,.28)]" style={{ transform: "perspective(170px) rotateX(62deg)", transformOrigin: "bottom" }} />
        <div className="absolute bottom-[33px] left-1/2 h-[30px] w-[96px] -translate-x-1/2 rounded-[8px] border border-white/[.08] bg-gradient-to-b from-[#424a43] to-[#2d342e]" style={{ transform: "perspective(150px) rotateX(58deg)", transformOrigin: "bottom" }} />
        <div className={`absolute bottom-[51px] left-1/2 h-[31px] w-[48px] -translate-x-1/2 rounded-[6px] border bg-[#101410] ${working ? "border-[#d9ff62]/45 shadow-[0_0_22px_rgba(217,255,98,.12)]" : "border-white/[.08]"}`}>
          <div className={`mx-auto mt-[7px] h-[14px] w-[32px] rounded-[3px] ${working ? "bg-[#d9ff62]/22" : "bg-white/[.04]"}`} />
        </div>
        <div className="absolute bottom-[12px] left-1/2 h-[26px] w-[34px] -translate-x-1/2 rounded-t-[14px] border border-white/[.08] bg-[#222923]" />
        <div className={`absolute left-1/2 top-0 flex h-[42px] w-[42px] -translate-x-1/2 items-center justify-center rounded-[13px] border border-white/[.08] bg-[#151a16] text-[21px] shadow-lg ${working && motion ? "animate-pulse" : ""}`}>{agent.emoji}</div>
        <span className={`absolute right-[13%] top-2 h-2.5 w-2.5 rounded-full ring-4 ring-[#101410] ${statusDot(agent.status)}`} />
      </div>
      <div className="truncate text-[11px] font-semibold text-white/88">{agent.name}</div>
      <div className="mt-0.5 truncate text-[8px] uppercase tracking-[.12em] text-white/24">{agent.department}</div>
      <div className="mt-1 inline-flex items-center gap-1 rounded-full bg-black/30 px-2 py-0.5 text-[8px] text-white/38"><span className={`h-1.5 w-1.5 rounded-full ${statusDot(agent.status)}`} />{statusLabel(agent.status)}</div>
    </button>
  );
}
...
export default function OfficePage() {
  ...
  const priorityRoster = useMemo(() => {
    ...
    const filler = sorted.filter((item) => !important.some((picked) => picked.id === item.id));
    return [...important, ...filler].slice(0, 6);
  ...
  return (
    <main className="min-h-[100dvh] w-full max-w-full overflow-x-hidden bg-[#0d100e] text-[#f3f5ee]">
      <header className="sticky top-0 z-40 w-full border-b border-white/[.07] bg-[#0d100e]/96 backdrop-blur-xl">
      ...
      <div className="mx-auto w-full max-w-6xl px-3 pb-20 pt-4 sm:px-5 md:pt-6">
        ...
        <section className="grid min-w-0 gap-4 lg:grid-cols-[1.1fr_.9fr] lg:items-end">
          <div className="min-w-0">
            ...
            <form onSubmit={submit} className="mt-4 w-full max-w-full overflow-hidden rounded-[20px] border border-white/10 bg-[#161a17] shadow-[0_18px_45px_rgba(0,0,0,.24)]">
              ...
              <div className="border-t border-white/[.07] bg-black/10 p-2.5">
                <div className="grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2">
                  <select value={workspaceId} onChange={(e) => setWorkspaceId(e.target.value)} className="block w-full min-w-0 max-w-full appearance-none rounded-xl border border-white/10 bg-[#222823] px-3 py-2.5 text-[11px] text-white/65 outline-none"><option value="">General workspace</option>{(overview?.workspaces || []).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
                  <select value={budgetMode} onChange={(e) => setBudgetMode(e.target.value as any)} className="block w-full min-w-0 max-w-full appearance-none rounded-xl border border-white/10 bg-[#222823] px-3 py-2.5 text-[11px] text-white/65 outline-none"><option value="economy">Economy</option><option value="balanced">Balanced</option><option value="max_quality">Max quality</option></select>
                </div>
        ...
        {tab === "floor" && <section className="mt-3 w-full max-w-full overflow-hidden rounded-[24px] border border-white/[.08] bg-[#141815] shadow-[0_28px_60px_rgba(0,0,0,.28)]">
          <div className="flex min-w-0 items-center justify-between gap-3 px-4 py-3"><div className="min-w-0"><div className="text-[8px] uppercase tracking-[.2em] text-white/25">The office</div><div className="mt-0.5 truncate text-[13px] font-semibold">Your team, live</div></div><div className="flex shrink-0 items-center gap-2"><button type="button" onClick={() => setMotion((value) => !value)} className="hidden items-center gap-1.5 text-[8px] text-white/32 sm:flex"><span>Motion</span><span className={`relative h-4 w-7 rounded-full ${motion ? "bg-[#d9ff62]/30" : "bg-white/10"}`}><span className={`absolute top-0.5 h-3 w-3 rounded-full bg-white transition ${motion ? "left-3.5" : "left-0.5"}`} /></span></button>{missionId && <Link href={`/office/missions/${missionId}`} className="inline-flex items-center gap-1 rounded-lg border border-white/10 px-2 py-1.5 text-[8px] text-white/38">Mission <ChevronRight className="h-3 w-3" /></Link>}</div></div>
          ...
          <div className="sm:hidden">
            <div className="grid grid-cols-2 gap-2 px-3 pb-3">
              {priorityRoster.map((base) => { const agent = agents[base.id]; return <MobileAgentCard key={agent.id} agent={agent} onOpen={() => setExpanded(agent.id)} />; })}
            </div>
            <div className="border-t border-white/[.05] px-3 py-3"><button type="button" onClick={() => setShowFullOffice((value) => !value)} className="w-full rounded-xl border border-white/[.07] bg-black/15 px-3 py-2.5 text-[9px] text-white/40">{showFullOffice ? "Show priority team" : `View all ${roster.length} agents`}</button></div>
          </div>

          <div className="relative hidden overflow-hidden border-y border-white/[.045] bg-[#101410] px-4 pb-4 pt-5 sm:block" style={{ backgroundImage: "linear-gradient(30deg,rgba(255,255,255,.014) 12%,transparent 12.5%,transparent 87%,rgba(255,255,255,.014) 87.5%,rgba(255,255,255,.014)),linear-gradient(150deg,rgba(255,255,255,.014) 12%,transparent 12.5%,transparent 87%,rgba(255,255,255,.014) 87.5%,rgba(255,255,255,.014))", backgroundSize: "40px 70px" }}>
            <div className="grid grid-cols-3 gap-2 lg:grid-cols-4 xl:grid-cols-5">{priorityRoster.map((base) => { const agent = agents[base.id]; return <DesktopStation key={agent.id} agent={agent} selected={expanded === agent.id} motion={motion} onSelect={() => setExpanded(agent.id)} />; })}</div>
            <div className="mt-2 flex justify-center"><button type="button" onClick={() => setShowFullOffice((value) => !value)} className="rounded-full border border-white/[.07] bg-black/20 px-3 py-1.5 text-[8px] text-white/34">{showFullOffice ? "Show priority team" : `View full office · ${roster.length} desks`}</button></div>
          </div>

          <div className="grid gap-2 border-t border-white/[.05] p-3 sm:grid-cols-2">
            <div className="min-w-0 rounded-xl border border-white/[.06] bg-white/[.018] p-3"><div className="flex items-center justify-between"><div className="text-[8px] uppercase tracking-[.15em] text-white/25">Chief</div><div className="inline-flex items-center gap-1 text-[8px] text-white/30"><span className={`h-1.5 w-1.5 rounded-full ${statusDot(agents.chief.status)}`} />{statusLabel(agents.chief.status)}</div></div><div className="mt-1.5 line-clamp-2 break-words text-[11px] font-medium leading-5 text-white/72">{agents.chief.detail}</div></div>
            <div className="grid grid-cols-3 gap-1.5"><div className="rounded-xl bg-black/20 p-2.5"><div className="text-[16px] font-semibold">{workingCount}</div><div className="text-[7px] text-white/25">working</div></div><div className="rounded-xl bg-black/20 p-2.5"><div className="text-[16px] font-semibold">{activeAgents.filter((a) => a.status === "completed").length}</div><div className="text-[7px] text-white/25">done</div></div><div className="rounded-xl bg-black/20 p-2.5"><div className="text-[16px] font-semibold">{attentionCount}</div><div className="text-[7px] text-white/25">attention</div></div></div>
          </div>
        </section>}
