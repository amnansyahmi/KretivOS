...
function SettingsPanel({ className, settings, setSettings, status, customers, onClose }: { className?: string; settings: Settings; setSettings: React.Dispatch<React.SetStateAction<Settings>>; status: Status; customers: { id: string; name: string }[]; onClose: () => void }) {
  return <aside className={cn("border-l bg-[#fbfaf7] p-4", className)}><div className="flex items-center justify-between"><div><div className="text-sm font-semibold">AI configuration</div><div className="mt-1 text-[10px] text-muted-foreground">Per-conversation controls</div></div><button className="xl:hidden" onClick={onClose}><X className="h-5 w-5" /></button></div>
    <div className="mt-6 space-y-6">
      <Field label="Client context"><select value={settings.customerId} onChange={(event) => setSettings((current) => ({ ...current, customerId: event.target.value }))} className="control"><option value="">Company-wide</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></Field>
      ...
      <Field label="Model"><select value={settings.model} onChange={(event) => setSettings((current) => ({ ...current, model: event.target.value }))} className="control"><option value="">Auto-route by mode</option>{status.models.map((model) => <option key={model} value={model}>{model.replace("ai-nonymauz-", "")}</option>)}</select></Field>
      ...
      {settings.useTools && <Field label="Default city"><input value={settings.city} onChange={(event) => setSettings((current) => ({ ...current, city: event.target.value }))} className="control" placeholder="Kuala Lumpur" /></Field>}
    ...
    <style jsx>{`.control{height:2.75rem;width:100%;border-radius:.75rem;border:1px solid hsl(var(--border));background:white;padding:0 .75rem;font-size:.8rem;outline:none}.control:focus{border-color:rgba(186,92,66,.6);box-shadow:0 0 0 4px rgba(186,92,66,.1)}`}</style>
  </aside>;
}
