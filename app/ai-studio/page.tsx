...
export default function AiStudioPage() {
  ...
  return <WorkspacePage
    ...
    actions={<div className="flex flex-wrap items-center gap-2"><ServiceBadge status={status} /><Button asChild variant="outline" className="bg-card"><Link href="/?view=Prompt%20Lab"><WandSparkles className="h-4 w-4" />Prompt Lab</Link></Button><Button variant="outline" className="bg-card" onClick={() => load(activeId)} disabled={loading}><RefreshCw className={cn("h-4 w-4", loading && "animate-spin")} />Refresh</Button><Button onClick={newConversation}><Plus className="h-4 w-4" />New</Button></div>}
  >
    ...
    <div className="grid min-h-[720px] overflow-hidden rounded-2xl border bg-card shadow-sm xl:grid-cols-[310px_minmax(0,1fr)_300px]">
      <aside className={cn("border-r bg-foreground text-white", mobileList ? "fixed inset-0 z-[160] block overflow-y-auto" : "hidden xl:block")}>
        <div className="sticky top-0 z-10 border-b border-white/10 bg-foreground p-4">
      ...
      <section className="flex min-w-0 flex-col bg-background">
        <div className="flex min-h-[70px] flex-wrap items-center gap-x-3 gap-y-2 border-b bg-white/70 px-3 py-3 sm:flex-nowrap sm:px-5">
          ...
          <div className="order-last flex w-full rounded-lg border bg-card p-1 sm:order-none sm:w-auto"><button onClick={() => setStudioMode("chat")} className={cn("flex-1 rounded-md px-3 py-1.5 text-xs sm:flex-none", studioMode === "chat" ? "bg-foreground text-white" : "text-muted-foreground")}><MessageSquareText className="mr-1.5 inline h-3.5 w-3.5" />Chat</button><button onClick={() => setStudioMode("image")} className={cn("flex-1 rounded-md px-3 py-1.5 text-xs sm:flex-none", studioMode === "image" ? "bg-foreground text-white" : "text-muted-foreground")}><ImageIcon className="mr-1.5 inline h-3.5 w-3.5" />Image</button></div>
          <Button variant="outline" size="icon" className="bg-card xl:hidden" onClick={() => setShowSettings(true)} aria-label="Settings"><Settings2 className="h-4 w-4" /></Button>
        ...
        {studioMode === "chat" ? <>
          <div className="flex-1 overflow-y-auto p-3 sm:p-5">
            ...
            <div className="mx-auto max-w-4xl space-y-5">{messages.map((message) => <ChatMessage key={message.id} message={message} onSave={() => saveMessage(message)} onFeedback={(value) => feedback(message.id, value)} />)}{sending && <div className="flex items-center gap-3 text-sm text-muted-foreground"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-foreground text-white"><Bot className="h-4 w-4" /></div><Loader2 className="h-4 w-4 animate-spin" />ai-nonymauz-cloud is working…</div>}<div ref={bottomRef} /></div>
          </div>
          <div className="border-t bg-white/80 p-3 sm:p-4"><div className="mx-auto max-w-4xl"><div className="rounded-2xl border bg-card p-2 shadow-sm focus-within:border-accent/60 focus-within:ring-4 focus-within:ring-accent/10"><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); send(); } }} className="min-h-20 w-full resize-none bg-transparent px-3 py-2 text-sm leading-6 outline-none" placeholder="Ask about Kretivco, a client, current web information, or generate work…" /><div className="flex flex-wrap items-center gap-2 px-1"><ContextChip icon={Database} active={settings.useCompanyData} label="KretivOS" /><ContextChip icon={Library} active={settings.useCloudRag} label="Cloud RAG" /><ContextChip icon={Globe2} active={settings.useTools} label="Tools" /><span className="ml-auto text-[10px] text-muted-foreground">Enter to send · Shift+Enter for new line</span><Button onClick={send} disabled={!prompt.trim() || sending}><Send className="h-4 w-4" />Send</Button></div></div></div></div>
      ...
      <SettingsPanel className={cn(showSettings ? "fixed inset-0 z-[160] block overflow-y-auto bg-background" : "hidden", "xl:static xl:block")} settings={settings} setSettings={setSettings} status={status} customers={data.customers} onClose={() => setShowSettings(false)} />
    </div>
  </WorkspacePage>;
}
...
function ContextChip({ icon: Icon, active, label }: { icon: any; active: boolean; label: string }) { return <span className={cn("inline-flex items-center gap-1 rounded-full px-2 py-1 text-[9px] font-medium", active ? "bg-emerald-50 text-emerald-700" : "bg-muted text-muted-foreground")}><Icon className="h-3 w-3" />{label}</span>; }

function Welcome({ templates, onSelect }: { templates: Template[]; onSelect: (template: Template) => void }) {
  return <div className="mx-auto flex max-w-3xl flex-col justify-center py-6 sm:min-h-[420px] sm:py-8"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-foreground text-white"><Sparkles className="h-5 w-5" /></div><h2 className="mt-5 text-2xl font-semibold tracking-tight">What should Kretiv AI work on?</h2><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Choose a proven workflow or start with your own request. Context and tools are controlled from the settings panel.</p><div className="mt-6 grid gap-3 sm:grid-cols-2">{templates.map((template) => <button key={template.id} onClick={() => onSelect(template)} className="rounded-xl border bg-card p-4 text-left transition hover:border-accent/50 hover:bg-card"><div className="flex items-center gap-2 text-sm font-semibold"><WandSparkles className="h-4 w-4 text-accent" />{template.name}</div><div className="mt-2 text-xs leading-5 text-muted-foreground">{template.description}</div></button>)}</div></div>;
}

function ChatMessage({ message, onSave, onFeedback }: { message: Message; onSave: () => void; onFeedback: (value: "up" | "down") => void }) {
  if (message.role === "user") return <div className="ml-auto max-w-[88%] rounded-2xl rounded-br-md bg-foreground px-4 py-3 text-sm leading-6 text-white sm:max-w-[75%]">{message.content}</div>;
  return <div className="group flex items-start gap-3"><div className="mt-1 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-foreground text-white"><Bot className="h-4 w-4" /></div><div className="min-w-0 flex-1"><div className="rounded-2xl border bg-card p-4 sm:p-5"><MarkdownPreview content={message.content} />{message.sources.length > 0 && <div className="mt-5 border-t pt-4"><div className="mb-2 text-[9px] font-semibold uppercase tracking-wider text-muted-foreground">KretivOS sources</div><div className="flex flex-wrap gap-2">{message.sources.map((source) => <a key={`${message.id}-${source.index}`} href={`/knowledge?id=${encodeURIComponent(source.id)}`} className="rounded-lg border bg-card px-2.5 py-2 text-[10px] hover:bg-card"><span className="font-semibold text-accent">[{source.index}]</span> {source.title}</a>)}</div></div>}</div><div className="mt-2 flex flex-wrap items-center gap-1 text-[10px] text-muted-foreground"><span>{message.model || message.mode}</span>{message.usage?.total_tokens ? <span>· {compact(message.usage.total_tokens)} tokens</span> : null}<div className="ml-auto flex gap-1"><button onClick={() => navigator.clipboard.writeText(message.content)} className="rounded-lg p-2 hover:bg-black/5" title="Copy"><Copy className="h-3.5 w-3.5" /></button><button onClick={onSave} className="rounded-lg p-2 hover:bg-black/5" aria-label="Save"><Save className="h-3.5 w-3.5" /></button><button onClick={() => onFeedback("up")} className={cn("rounded-lg p-2 hover:bg-black/5", message.feedback === "up" && "bg-emerald-50 text-emerald-700")}><ThumbsUp className="h-3.5 w-3.5" /></button><button onClick={() => onFeedback("down")} className={cn("rounded-lg p-2 hover:bg-black/5", message.feedback === "down" && "bg-red-50 text-red-700")}><ThumbsDown className="h-3.5 w-3.5" /></button></div></div></div></div>;
}
...
function SettingsPanel({ className, settings, setSettings, status, customers, onClose }: { className?: string; settings: Settings; setSettings: React.Dispatch<React.SetStateAction<Settings>>; status: Status; customers: { id: string; name: string }[]; onClose: () => void }) {
  return <aside className={cn("border-l bg-card p-4", className)}><div className="flex items-center justify-between"><div><div className="text-sm font-semibold">AI configuration</div><div className="mt-1 text-[10px] text-muted-foreground">Per-conversation controls</div></div><button className="xl:hidden" onClick={onClose}><X className="h-5 w-5" /></button></div>
    <div className="mt-6 space-y-6">
      ...
      <div><div className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Reasoning mode</div><div className="space-y-2">{modes.map((mode) => <button key={mode.id} onClick={() => setSettings((current) => ({ ...current, mode: mode.id }))} className={cn("flex w-full items-center justify-between rounded-xl border px-3 py-3 text-left", settings.mode === mode.id ? "border-accent bg-card" : "bg-card")}><span className="text-sm font-medium">{mode.label}</span><span className="text-[10px] text-muted-foreground">{mode.note}</span></button>)}</div></div>
...
function Toggle({ icon: Icon, label, note, checked, onChange }: { icon: any; label: string; note: string; checked: boolean; onChange: (value: boolean) => void }) { return <button onClick={() => onChange(!checked)} className="flex w-full items-center gap-3 rounded-xl border bg-card p-3 text-left"><div className={cn("flex h-9 w-9 items-center justify-center rounded-lg", checked ? "bg-foreground text-white" : "bg-muted text-muted-foreground")}><Icon className="h-4 w-4" /></div><div className="min-w-0 flex-1"><div className="text-xs font-medium">{label}</div><div className="mt-0.5 truncate text-[9px] text-muted-foreground">{note}</div></div><div className={cn("flex h-5 w-9 items-center rounded-full p-0.5 transition", checked ? "justify-end bg-emerald-500" : "justify-start bg-well")}><span className="h-4 w-4 rounded-full bg-card shadow" /></div></button>; }
...
function ImageStudio({ status, prompt, setPrompt, style, setStyle, image, loading, onGenerate }: { status: Status; prompt: string; setPrompt: (value: string) => void; style: string; setStyle: (value: string) => void; image: { dataUrl: string; promptUsed: string; model: string; usageToday: number; dailyLimit: number } | null; loading: boolean; onGenerate: () => void }) {
  ...
  return <div className="grid flex-1 gap-5 overflow-y-auto p-4 lg:grid-cols-[minmax(0,420px)_minmax(0,1fr)] lg:p-6"><div><div className="text-[10px] font-semibold uppercase tracking-[.18em] text-accent">Pollinations Flux</div><h2 className="mt-2 text-2xl font-semibold">Generate an image</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Uses the image endpoint already deployed inside ai-nonymauz-cloud. The generated file can be downloaded directly.</p><label className="mt-6 block text-xs font-medium">Prompt<textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} className="mt-2 min-h-44 w-full resize-y rounded-xl border bg-card p-4 text-sm leading-6 outline-none focus:border-accent/60 focus:ring-4 focus:ring-accent/10" placeholder="Describe the exact visual, composition, lighting, subject and aspect intent…" /></label><div className="mt-4"><div className="text-xs font-medium">Style</div><div className="mt-2 flex flex-wrap gap-2">{styles.map((item) => <button key={item} onClick={() => setStyle(item)} className={cn("rounded-lg border px-3 py-2 text-xs capitalize", style === item ? "border-foreground bg-foreground text-white" : "bg-card")}>{item}</button>)}</div></div><Button className="mt-5 w-full" onClick={onGenerate} disabled={!prompt.trim() || loading}>{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <WandSparkles className="h-4 w-4" />}{loading ? "Generating…" : "Generate image"}</Button></div><div className="flex min-h-[430px] items-center justify-center rounded-2xl border bg-card p-3">{image ? <div className="w-full"><img src={image.dataUrl} alt={image.promptUsed} className="mx-auto max-h-[650px] w-full rounded-xl object-contain" /><div className="mt-3 flex items-center gap-2"><div className="min-w-0 flex-1"><div className="truncate text-xs font-medium">{image.model}</div><div className="mt-1 text-[10px] text-muted-foreground">Daily usage {image.usageToday}/{image.dailyLimit}</div></div><Button variant="outline" asChild><a href={image.dataUrl} download={`kretivos-${Date.now()}.png`}><Download className="h-4 w-4" />Download</a></Button></div></div> : <div className="max-w-sm text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-muted"><ImageIcon className="h-6 w-6 text-accent" /></div><div className="mt-4 font-semibold">Your generated visual appears here</div><p className="mt-2 text-xs leading-5 text-muted-foreground">Current service limit: {status.image?.daily_limit || 20} images per day.</p></div>}</div></div>;
}
