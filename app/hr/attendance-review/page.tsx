  ...
  return <main className="min-h-screen bg-[#f5f2ea] pb-24 text-[#202820]">
    <header className="sticky top-0 z-40 border-b border-black/5 bg-[#f5f2ea]/95 backdrop-blur-xl">
      <div className="mx-auto flex min-h-16 max-w-[1600px] items-center gap-3 px-4 md:min-h-24 md:px-8 md:py-5">
        <Button asChild variant="outline" size="icon" className="bg-white"><Link href="/hr" aria-label="Back to HR"><ArrowLeft className="h-4 w-4" /></Link></Button>
    ...
    <div className="mx-auto max-w-[1600px] space-y-5 px-4 py-5 md:px-8 md:py-7">
      <Card className="border-amber-200 bg-amber-50/80"><CardContent className="flex items-start gap-3 p-4 text-xs leading-5 text-amber-900"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /><span>This admin page is temporarily visible without login. Use it for internal testing only. Role-based access will restrict attendance evidence to HR/Admin later.</span></CardContent></Card>
      ...
      <Card className="border-black/8 bg-white/90"><CardContent className="p-4">
        <div className="flex items-center gap-2 text-sm font-semibold"><Filter className="h-4 w-4 text-[#ba5c42]" />Filters</div>
      ...
      <Card className="hidden overflow-hidden border-black/8 bg-white/90 lg:block">
        <div className="grid grid-cols-[1.45fr_.8fr_.72fr_.72fr_.78fr_.7fr_.52fr] gap-3 border-b bg-[#f8f6f0] px-5 py-3 text-[10px] font-semibold uppercase tracking-[.12em] text-muted-foreground">
        ...
        {records.map((record) => {
          ...
          return <button key={record.id} onClick={() => setSelected(record)} className="grid w-full grid-cols-[1.45fr_.8fr_.72fr_.72fr_.78fr_.7fr_.52fr] items-center gap-3 border-b px-5 py-4 text-left transition last:border-b-0 hover:bg-[#faf8f3]">
            <div className="flex min-w-0 items-center gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#202c25] text-xs font-semibold text-white">{initials(employee?.name || "U")}</div><div className="min-w-0"><div className="truncate text-sm font-semibold">{employee?.name || "Unknown"}</div><div className="mt-1 truncate text-[10px] text-muted-foreground">{record.workMode || record.status} · {employee?.department || "Team"}</div></div></div>
...
function Stat({ label, value, note, icon: Icon }: { label: string; value: number; note: string; icon: any }) {
  return <Card className="border-black/8 bg-white/90"><CardContent className="p-4"><div className="flex items-center justify-between"><div className="text-xs text-muted-foreground">{label}</div><Icon className="h-4 w-4 text-[#ba5c42]" /></div><div className="mt-2 text-2xl font-semibold">{value}</div><div className="mt-1 text-[10px] text-muted-foreground">{note}</div></CardContent></Card>;
}
...
function Mini({ label, value, photo }: { label: string; value: string; photo?: boolean }) {
  return <div className="rounded-xl bg-[#f7f4ed] p-3"><div className="flex items-center justify-between gap-1 text-[10px] text-muted-foreground"><span>{label}</span>{photo && <Camera className="h-3 w-3" />}</div><div className="mt-1 truncate text-xs font-semibold">{value}</div></div>;
}
...
function Detail({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl bg-[#f7f4ed] p-3"><div className="text-[10px] text-muted-foreground">{label}</div><div className="mt-1 break-words text-xs font-medium">{value}</div></div>;
}
