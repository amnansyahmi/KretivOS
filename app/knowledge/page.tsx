    ...
    <main className="min-h-screen bg-[#f5f2ea] pb-24 text-[#202820]">
      <header className="sticky top-0 z-40 border-b border-black/5 bg-[#f5f2ea]/95 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1500px] items-center gap-3 px-4 md:h-auto md:min-h-24 md:px-8 md:py-6">
          <Link href="/" className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-[#506057] hover:bg-black/5" aria-label="Back to KretivOS"><ArrowLeft className="h-5 w-5" /></Link>
        ...
        <div className="grid min-w-0 gap-5 lg:grid-cols-[360px_minmax(0,1fr)]">
          ...
          <section className={cn("min-w-0", screen === "library" && "hidden lg:block")}>
            {selected ? <div className="space-y-4">
                ...
                <CardContent className="p-4 md:p-7">
                  {editing && editDraft ? <div className="space-y-5">
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <Field label="Next review"><Input type="date" value={editDraft.nextReviewAt?.slice(0, 10) || ""} onChange={(event) => setEditDraft({ ...editDraft, nextReviewAt: event.target.value })} /></Field>
                    </div>
              ...
              <button onClick={() => setAiOpen(true)} className="hidden w-full items-center justify-between rounded-2xl border border-[#26342b]/10 bg-[#26342b] p-5 text-left text-white transition hover:bg-[#2d3d32] lg:flex"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-white/10"><Sparkles className="h-5 w-5 text-[#ef9a75]" /></div><div><div className="font-semibold">Ask Kretiv AI about this document</div><div className="mt-1 text-xs text-white/50">Uses this file and up to three related shared records as grounded context.</div></div></div><ChevronRight className="h-5 w-5 text-white/40" /></button>
