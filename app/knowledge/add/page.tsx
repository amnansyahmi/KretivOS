  ...
  return (
    <main className="min-h-screen bg-background pb-24 text-foreground">
      ...
      <div className="mx-auto max-w-[1500px] px-4 py-4 md:px-8 md:py-7">
        ...
        <div className="mb-4 flex rounded-xl border bg-card p-1 md:hidden"><button onClick={() => setTab("editor")} className={cn("flex h-10 flex-1 items-center justify-center gap-2 rounded-lg text-sm font-medium", tab === "editor" ? "bg-foreground text-white" : "text-muted-foreground")}><FileCode2 className="h-4 w-4" />Editor</button><button onClick={() => setTab("preview")} className={cn("flex h-10 flex-1 items-center justify-center gap-2 rounded-lg text-sm font-medium", tab === "preview" ? "bg-foreground text-white" : "text-muted-foreground")}><Eye className="h-4 w-4" />Preview</button></div>

        <div className="grid min-w-0 gap-5 lg:grid-cols-[340px_minmax(0,1fr)]">
          <aside className={cn("space-y-4", tab === "preview" && "hidden lg:block")}>
            <Card className="border-black/8 bg-white/90 shadow-sm"><CardContent className="p-4"><div className="flex items-center justify-between"><div><h2 className="font-semibold">Import Markdown</h2><p className="mt-1 text-xs text-muted-foreground">Maximum 2 MB</p></div><Upload className="h-5 w-5 text-accent" /></div><input ref={inputRef} type="file" accept=".md,text/markdown,text/plain" className="hidden" onChange={(event) => handleFile(event.target.files?.[0])} /><button onClick={() => inputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); handleFile(event.dataTransfer.files?.[0]); }} className="mt-4 flex w-full items-center gap-3 rounded-2xl border border-dashed bg-card p-4 text-left transition hover:border-accent/60 hover:bg-card"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-muted"><Upload className="h-5 w-5 text-accent" /></div><div><div className="text-sm font-semibold">Choose or drop .md</div><div className="mt-1 text-xs text-muted-foreground">Title is detected from the first H1.</div></div></button></CardContent></Card>
            ...
            <Card className="border-black/8 bg-white/90 shadow-sm"><CardContent className="space-y-4 p-4"><div><h2 className="font-semibold">Knowledge details</h2><p className="mt-1 text-xs text-muted-foreground">Used by filters, relationships and AI grounding.</p></div>
              <Field label="Title"><Input value={title} onChange={(event) => { setTitle(event.target.value); setSavedEntry(null); }} /></Field>
              <Field label="Customer / workspace"><Select value={customerId} onChange={(event) => { setCustomerId(event.target.value); setBrandId(""); setSavedEntry(null); }} ><option value="">Kretivco / Internal</option>{customers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></Field>
              <Field label="Brand"><Select value={brandId} onChange={(event) => { setBrandId(event.target.value); setSavedEntry(null); }} disabled={!customerId}><option value="">No specific brand</option>{availableBrands.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></Field>
              <Field label="Category"><Select value={category} onChange={(event) => { setCategory(event.target.value); setSavedEntry(null); }} >{categoryOptions.map((name) => <option key={name}>{name}</option>)}</Select></Field>
              <Field label="Tags"><Input value={tags} onChange={(event) => { setTags(event.target.value); setSavedEntry(null); }} placeholder="campaign, SOP, finance" /></Field>
              <Field label="Filename"><Input value={filename} onChange={(event) => { setFilename(event.target.value); setSavedEntry(null); }} /></Field>
              <Field label="Knowledge owner"><Input value={owner} onChange={(event) => { setOwner(event.target.value); setSavedEntry(null); }} placeholder="Marketing, Finance or a team member" /></Field>
              <Field label="Original source URL"><Input type="url" value={sourceUrl} onChange={(event) => { setSourceUrl(event.target.value); setSavedEntry(null); }} placeholder="https://…" /></Field>
              <Field label="Review every"><Select value={reviewIntervalDays} onChange={(event) => { setReviewIntervalDays(event.target.value); setSavedEntry(null); }} ><option value="30">30 days</option><option value="60">60 days</option><option value="90">90 days</option><option value="180">180 days</option><option value="365">365 days</option></Select></Field>
              <div className="grid grid-cols-2 gap-2 rounded-xl bg-background p-3 text-xs"><div><div className="text-muted-foreground">Words</div><div className="mt-1 font-semibold">{wordCount}</div></div><div><div className="text-muted-foreground">Storage</div><div className="mt-1 font-semibold">Neon</div></div></div>
            </CardContent></Card>
          </aside>

          <Card className="min-w-0 overflow-hidden border-black/8 bg-white/90 shadow-sm"><div className="hidden items-center justify-between border-b bg-card p-4 md:flex"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-muted"><FileCode2 className="h-5 w-5 text-accent" /></div><div><h2 className="font-semibold">Markdown workspace</h2><p className="mt-1 text-xs text-muted-foreground">Edit and preview before saving.</p></div></div><div className="flex rounded-xl border bg-card p-1"><button onClick={() => setTab("editor")} className={cn("rounded-lg px-3 py-2 text-xs font-medium", tab === "editor" ? "bg-foreground text-white" : "text-muted-foreground")}>Editor</button><button onClick={() => setTab("preview")} className={cn("rounded-lg px-3 py-2 text-xs font-medium", tab === "preview" ? "bg-foreground text-white" : "text-muted-foreground")}>Preview</button></div></div><CardContent className="p-3 md:p-5">{tab === "editor" ? <textarea value={content} onChange={(event) => { setContent(event.target.value); setSavedEntry(null); }} className="min-h-[64dvh] w-full resize-y rounded-xl border bg-card p-4 font-mono text-sm leading-6 outline-none focus:border-accent focus:ring-4 focus:ring-accent/10 md:min-h-[650px]" /> : <div className="min-h-[64dvh] rounded-xl border bg-card p-4 md:min-h-[650px] md:p-7"><div className="mx-auto max-w-4xl"><MarkdownPreview content={content} /></div></div>}</CardContent></Card>
        </div>
