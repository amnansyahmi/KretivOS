...
/** Migration remains the canonical schema; this makes preview/production safe when a deployment lands first. */
export function ensureOfficeSchema() {
  if (schemaReady) return schemaReady;
  schemaReady = (async () => {
    const sql = getDatabase();
    await sql`create table if not exists ai_office_missions (
    ...
    await sql`create table if not exists ai_office_tasks (
      ...
      input_request text, human_input text, created_at timestamptz not null default now(), started_at timestamptz,
      completed_at timestamptz, updated_at timestamptz not null default now(), unique(mission_id, task_key)
    )`;
    await sql`create table if not exists ai_office_agent_runs (
      id uuid primary key default gen_random_uuid(), mission_id uuid not null references ai_office_missions(id) on delete cascade,
...
export async function createOfficeMission(mission: string) {
  ...
  const rows = await sql`insert into ai_office_missions (organization_id, title, mission, status, started_at)
    values (${ORGANIZATION_ID}, ${title}, ${mission}, 'planning', now()) returning id::text`;
...
export async function saveOfficePlan(missionId: string, plan: OfficePlan, grounded: boolean, sourceCount: number) {
  await ensureOfficeSchema();
  const sql = getDatabase();
  await sql`update ai_office_missions set mission_type=${plan.missionType}, objective=${plan.objective}, summary=${plan.summary},
    grounded=${grounded}, source_count=${sourceCount}, status='running', updated_at=now() where id=${missionId}::uuid`;
  for (const task of plan.tasks) {
    await sql`insert into ai_office_tasks (mission_id, task_key, agent_id, title, instruction, depends_on, status)
      ...
      instruction=excluded.instruction, depends_on=excluded.depends_on, status='queued', updated_at=now()`;
  }
}
...
export async function setOfficeTaskStatus(missionId: string, taskKey: string, status: string, output?: string, inputRequest?: string) {
  const sql = getDatabase();
  await sql`update ai_office_tasks set status=${status}, output=coalesce(${output ?? null},output),
...
export async function addOfficeAgentRun(missionId: string, agentId: string, status: string, detail: string, taskKey?: string, output?: string, durationMs?: number) {
  const sql = getDatabase();
  await sql`insert into ai_office_agent_runs (mission_id,task_id,agent_id,status,detail,output,duration_ms)
    values (${missionId}::uuid,(select id from ai_office_tasks where mission_id=${missionId}::uuid and task_key=${taskKey || ""} limit 1),
      ${agentId},${status},${detail},${output ?? null},${durationMs ?? null})`;
}

export async function pauseOfficeMission(missionId: string) {
  const sql = getDatabase();
  await sql`update ai_office_missions set status='waiting_input',updated_at=now() where id=${missionId}::uuid`;
}
...
export async function listOfficeMissions(limit = 20) {
  await ensureOfficeSchema();
  const sql = getDatabase();
  return await sql`select id::text,title,mission_type,objective,status,grounded,source_count,created_at::text,completed_at::text,
    ...
    from ai_office_missions m where organization_id=${ORGANIZATION_ID} order by created_at desc limit ${Math.max(1,Math.min(limit,100))}`;
}
